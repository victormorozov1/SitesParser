...
class TestApi:
    ...
    def test_create_regexp_parser_endpoint(self, mocker: MockFixture) -> None:
        create_regexp_parser = mocker.patch('app.api.create_regexp_parser', return_value=4)
        response = self.client.post(
            '/create_regexp_parser', headers={'regexp': 'some_regexp', 'parser_type': 'GROUP'},
        )
        ...
        response = self.client.post(
            '/create_regexp_parser', headers={'regexp': 'some_regexp', 'parser_type': 'pipi'},
        )
